...
template <typename T> Eigen::Matrix<T,3,3> EulerToRotM(const Eigen::Matrix<T,3,1>& ang)
{
    const T x = ang(0);  // roll
    const T y = ang(1);  // pitch
    const T z = ang(2);  // yaw

    const T cx = std::cos(x), sx = std::sin(x);
    const T cy = std::cos(y), sy = std::sin(y);
    const T cz = std::cos(z), sz = std::sin(z);
...
template <typename T>
inline Eigen::Quaternion<T> EulerToQuat(T roll, T pitch, T yaw)
{
    // R = Rz(yaw) * Ry(pitch) * Rx(roll)
    Eigen::AngleAxis<T> Rx(roll,  Eigen::Matrix<T,3,1>::UnitX());
    Eigen::AngleAxis<T> Ry(pitch, Eigen::Matrix<T,3,1>::UnitY());
    Eigen::AngleAxis<T> Rz(yaw,   Eigen::Matrix<T,3,1>::UnitZ());
    return Rz * Ry * Rx;   // 隐式构造成 Quaternion<T>
}
